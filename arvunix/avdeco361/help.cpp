...
static char *genmesg [] = {
	"The Demos Commander, Copyright (C) 1989-1994 Serge Vakulenko",
	" Cursor Arrows ",
	"Left             ^[ l",
	"Right            ^[ r",
	"Up               ^[ u",
...
static char *editmesg [] = {
	"The Demos Commander Text Editor, Copyright (C) 1990-1994 Serge Vakulenko",
	" Cursor Movement ",
	"Char Left        ^A",
	"Char Right       ^D",
	"Line Up          ^E",
...
static char *viewmesg [] = {
	"The Demos Commander Viewer, Copyright (C) 1990-1994 Serge Vakulenko",
	" Cursor Movement ",
	"Page Left        ^A",
	"Page Right       ^D",
	"Line Up          ^E",
...
static char *hviewmesg [] = {
	"The Demos Commander Hexadecimal Viewer, Copyright (C) 1990-1994 Serge Vakulenko",
	" Cursor Movement ",
	"Page Left        ^A",
	"Page Right       ^D",
	"Line Up          ^E",
...
/* local */
void initnewhelp (register char **q, register char *p) {
	for (; *q; ++q) {
		for(*q=p; *p && *p != '\n'; ++p)
			if (*p == '@')
				*p = ' ';
...
/* local */
int findhelpfile (char *filename, char *name, char *msg, char *dir) {
	register char *s;

	s = filename;                   /* build dir/[msg.]name */
	strcpy (s, dir);
	s += strlen (s);
	if (msg && *msg) {
		*s++ = *msg;
...
void inithelp (struct help *hp) {
	...
	m = EnvGet ("MSG");
	findhelpfile (filename, hp->name, m, LCLHELPDIR) ||
	    findhelpfile (filename, hp->name, m, HELPDIR);
	fd = open (filename, 0);
	if (fd >= 0) {
