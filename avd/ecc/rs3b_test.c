...
static rs3w *ErrorTable = NULL, *TestTable = NULL;
static dword NumElemErrorTable = 0, RecurCounter = 0, pErrorTable = 0;

static dword NumOne (rs3w *mask)
{
  dword i, num = 0;

  for (i = 0; i < RS3_CODE_LEN; i++)
    if (mask->rs [i])
      num++;
...
static bool CompareMask (rs3w *src, rs3w *dst)
{
  dword i;
...
static void RecurProc (rs3w *mask, dword Nested)
{
...
static void RecalcErrorTable (void)
{
  ...
  NumElemErrorTable += RS3_CODE_LEN/1;
//  NumElemErrorTable += ((RS3_CODE_LEN - 1) * (RS3_CODE_LEN))/2;
//  NumElemErrorTable += ((RS3_CODE_LEN - 2) * (RS3_CODE_LEN - 1) * (RS3_CODE_LEN))/6;
  /* NumElemErrorTable = 11155 + 1; */ /* without checking double */
  printf ("-NumElemErrorTable = %d\n", NumElemErrorTable);

  if (!(ErrorTable = malloc (sizeof (rs3w) * NumElemErrorTable)) )
    ErrorExit(1);

  if ((hFerrtabl = open (ErrorName, O_BINARY | O_RDWR, FILE_ACCESS)) == -1)
   {
    memset (ErrorTable, 0, NumElemErrorTable * sizeof (rs3w));

    for (i = 1; i <= RS3_MAX_RESTORE_ERROR; i++)
     {
      rs3w loc;
    ...
    write (hFerrtabl, ErrorTable, NumElemErrorTable * sizeof (rs3w));
   }
  else
    read (hFerrtabl, ErrorTable, NumElemErrorTable * sizeof (rs3w));
...
static void TestError (void)
{
  ...
  if (!(TestTable = malloc (sizeof(rs3w) * NumElemErrorTable)))
    ErrorExit (1);
  ...
  while (errortempl--)
   {
    for (j = 0; j < (1 << RS3_VALUE_LEN); j++)
     {
      memset (TestTable, 0, NumElemErrorTable * sizeof (rs3w));
	...
	for (k = 0; k < KK; k++)
      ...
      for (i = 0; i < NumElemErrorTable; i++)
       {
	encode_rs3 (TestTable [i].rs, &TestTable [i].rs [KK]); /* 9 */
	 ...
	 * eras_dec_rs3 (TestTable [i].rs, NULL, 0); */
	eras_dec_rs3_wers (TestTable [i].rs);
       }
      ...
      else
	{
	 ...
	 write (hTesttabl, TestTable, NumElemErrorTable * sizeof (rs3w));
	 close (hTesttabl);
	 ErrorExit (2);
	}
     } /* end for */
  ...
  printf ("\nSuccess!!!\n");
  Comb = NumElemErrorTable * (1 << RS3_VALUE_LEN) * 0xf;
...
int main (void)
{
  printf ("Reed-Solomon ECC (%d,%d) coder/encoder test program.\n", RS3_CODE_LEN, RS3_VALUE_LEN);

  InitECCTable ();
  RecalcErrorTable ();
  TestError ();
  
  ECCTest (500, 500, RS3_FLAG | ARVID1031_FLAG | CLUSTER_BLOCK);
  ECCTest (500 * RS3_VALUE_LEN, 500 * RS3_VALUE_LEN, RS3_FLAG | ARVID1031_FLAG);
