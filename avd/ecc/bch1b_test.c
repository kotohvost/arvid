...
#if 0
static rsw *ErrorTable = NULL, *TestTable = NULL;
static dword NumElemErrorTable = 0, RecurCounter = 0, pErrorTable = 0;

static dword NumOne (rsw *mask)
{
  dword i, num = 0;

  for (i = 0; i < RS1_CODE_LEN; i++)
    if (mask->rs [i])
      num++;
...
static bool CompareMask (rsw *src, rsw *dst)
{
  dword i;
...
static void RecurProc (rsw *mask, dword Nested)
{
...
static void RecalcErrorTable (void)
{
  ...
  NumElemErrorTable += RS1_CODE_LEN/1;
  NumElemErrorTable += ((RS1_CODE_LEN - 1) * (RS1_CODE_LEN))/2;
  NumElemErrorTable += ((RS1_CODE_LEN - 2) * (RS1_CODE_LEN - 1) * (RS1_CODE_LEN))/6;
  /* NumElemErrorTable = 11155 + 1; */ /* without checking double */
  printf ("-NumElemErrorTable = %d\n", NumElemErrorTable);

  if (!(ErrorTable = malloc (sizeof (rsw) * NumElemErrorTable)) )
    ErrorExit(1);

  if ((hFerrtabl = open (ErrorName, O_BINARY | O_RDWR, FILE_ACCESS)) == -1)
   {
    memset (ErrorTable, 0, NumElemErrorTable * sizeof (rsw));

    for (i = 1; i <= RS1_MAX_RESTORE_ERROR; i++)
     {
      rsw loc;
    ...
    write (hFerrtabl, ErrorTable, NumElemErrorTable * sizeof (rsw));
   }
  else
    read (hFerrtabl, ErrorTable, NumElemErrorTable * sizeof (rsw));
...
static void TestError (void)
{
  ...
  if (!(TestTable = malloc (sizeof(rsw) * NumElemErrorTable)))
    ErrorExit (1);
  ...
  while (errortempl--)
   {
    for (j = 0; j < (1 << RS1_VALUE_LEN); j++)
     {
      memset (TestTable, 0, NumElemErrorTable * sizeof (rsw));
	...
	for (k = 0; k < KK; k++)
      ...
      for (i = 0; i < NumElemErrorTable; i++)
       {
	encode_rs (TestTable [i].rs, &TestTable [i].rs [KK]); /* 9 */
	 ...
	 * eras_dec_rs (TestTable [i].rs, NULL, 0); */
	eras_dec_rs_wers (TestTable [i].rs);
       }
      ...
      else
	{
	 ...
	 write (hTesttabl, TestTable, NumElemErrorTable * sizeof (rsw));
	 close (hTesttabl);
	 ErrorExit (2);
	}
     } /* end for */
  ...
  printf ("\nSuccess!!!\n");
  Comb = NumElemErrorTable * (1 << RS1_VALUE_LEN) * 0xf;
...
int main (void)
{
  printf ("BCH ECC (%d,%d) coder/encoder test program.\n", BCH1_CODE_LEN, BCH1_VALUE_LEN);

  InitECCTable ();
/*  RecalcErrorTable ();
  TestError (); */
  
  ECCTest (50, 50, BCH1_FLAG | ARVID1031_FLAG | CLUSTER_BLOCK);
  ECCTest (50 * BCH1_VALUE_LEN, 50 * BCH1_VALUE_LEN, BCH1_FLAG | ARVID1031_FLAG);
