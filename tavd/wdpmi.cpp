 ...
 *     Copyright (C) 1999,2000 UltraVision Ent.
 ************************************************************************/
...
int __dpmi_allocate_dos_memory(int _paragraphs, int *_ret_selector_or_max)/* DPMI 0.9 AX=0100 */
{
    union REGS r;
    r.w.ax = 0x0100;
    r.w.bx = (short) (_paragraphs);
    int386 (DPMI_INT,&r,&r);
    *_ret_selector_or_max = r.w.dx;
    r.x.eax = r.w.ax;
    __tb = r.x.eax << 4; //convert to linear address
    return r.x.cflag;
}
...
int __dpmi_get_protected_mode_interrupt_vector(int _vector, __dpmi_paddr *_address)/* DPMI 0.9 AX=0204 */
{
    ...
    _address->selector = r.w.cx;
    _address->offset32 = r.x.edx;
    return r.x.cflag;
}
int __dpmi_set_protected_mode_interrupt_vector(int _vector, __dpmi_paddr *_address)/* DPMI 0.9 AX=0205 */
{
    ...
    r.w.ax = 0x0205;
    r.h.bl = _vector;
    r.w.cx = _address->selector;
    r.x.edx = _address->offset32;
...
int __dpmi_get_version(__dpmi_version_ret *_ret)/* DPMI 0.9 AX=0400 */
{
    ...
    _ret->flags      =	r.w.bx;
    _ret->cpu	     =	r.h.cl;
    _ret->master_pic =	r.h.dh;
    _ret->slave_pic  =	r.h.dl;
    return r.x.cflag;
}
...
int __dpmi_physical_address_mapping (__dpmi_meminfo *_info)/* DPMI 0.9 AX=0800 */
{
    ...
    int386 (DPMI_INT, &r, &r);
    _info->address = ((unsigned long)r.w.bx << 16) + (unsigned long)r.w.cx;
    return r.x.cflag;
}
