...
/*
 * count - умнажается на 2
 */
int ReadWriteArv (avt_unit *t, void *buf, u_int count)
{
u_int	c, b;
/* Внимательно следить за флагом dmaflags */
	isa_dmastartblk (t->dmaflags, (caddr_t)buf, count * 2, t->drq);
/* вставить проверку на тайм-аут по записи/чтению */
	b = 0;
/* если дма на автомате, то програмный запрос не должон устанавливаться */
XRWA:
	for (c=0; c < 0x1388; c++) if (isa_dmaCount(t->drq) == 0) return (0);
/* выставим программный запрос */
	outb (DMA2_REQ, t->drq);
	DELAY(100);
	outb (DMA2_REQ, (t->drq & 3));
	b ^= 1;
	if (b) goto XRWA;
	return (c);
} 
...
/*
 * старший бит в слове равен 1
 * или команды 8 и 4 - выход
 */
...
/* вывод на излучатель */
	...
	case 0:
	dafault: i = 2; strsize = 20; break;
	}
/* вывести две команды в арвид */
	while (i > 0 ) { ReadWriteArv (t, (caddr_t)commdma, strsize); i--; }
/* общий выход */
commdone:
	t->dmaflags |= B_RAW;
}
...
u_short	chkdma[20];
	...
	t->dmaflags = B_WRITE;
	ReadWriteArv (t, (caddr_t)chkdma, 10);
/*	isa_dmareq (t->drq); */
	ReadWriteArv (t, (caddr_t)chkdma, 20);
/*	isa_dmareq (t->drq); */
	ReadWriteArv (t, (caddr_t)chkdma, 20);
	for (i=0; i<2; i++)
	{
	 if (ReadWriteArv (t, (caddr_t)chkdma, 20) > 0) return (0);
...
/* главное чтоб в памяти небыло таких слов */
	...
	t->dmaflags = B_READ;
	if (ReadWriteArv (t, (caddr_t)tstdma, 1000) > 0)
...
/*	 t->dmacount = SIZE1020; */
	 t->dmacount = 284 * 9 * 8;
	 t->buf = malloc (t->dmacount, M_DEVBUF, M_NOWAIT);
