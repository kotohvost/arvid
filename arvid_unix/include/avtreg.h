...
#define	SIZE100		284*8*2		/* размер кадра записи 100 K/s */
#define	SIZE200		284*9*2		/* размер кадра записи 200 K/s */
#define	SIZE325		296*13*2	/* размер кадра записи 325 K/s */
...
/* 1020, 1030 - Rid-Solomon, поле nsect */
typedef struct  {
	U8	format;		/* формат ленты */
	U8	nsect;		/* число секторов в кадре */
	U8	name[8];	/* имя ленты, в оригинале I8 (?) */
	U16	ident;		/* индентификатор ленты */
	U16	length;		/* длина ленты, аттрибуты начала/конца записи */
#define	MASKLEN		0x03ff	/* маска для определения длины ленты */
#define	SPEC_ATTR	0x1000	/* атрибут записи log или метка XOR кадра (?) */
#define	TAIL_ATTR	0x2000	/* атрибут конца записи */
#define	HEAD_ATTR	0x4000	/* атрибут начала записи */
#define	XOR_HAS		0x8000	/* атрибут записи содержащей XOR кадр (?) */
	U16	tapetime;	/* время от начала ленты, sec */
	I32	sector;		/* начальный сектор */
#define	MASKSECT200	0x3fffffff	/* маска для определения номера сектора */
#define	DIRECTORY200	0x80000000	/* аттрибут записи директории */
...
/* 1050 - Rid-Solomon, 325 K/s */
typedef struct  {
	U8	format;		/* формат ленты */
	U8	nsect;		/* число секторов в кадре */
	U16	lenCompress;	/* максимальный размер сектора после сжатия */
#define	NO_COMPRESS	0x8000	/* компрессия отсутствует */
	U32	ident;		/* индентификатор ленты */
	U32	reserv0;	/* 0 */
	U16	length;		/* длина ленты, аттрибуты записи */
#define	DIRECTORY_325	0x0800	/* атрибут директории на ленте */
	U16	tapetime;	/* время от начала ленты, sec */
	I32	sector;		/* начальный сектор */
	U32	reserv1;	/* 0xffffffff */
	U32	tstsum;		/* контрольная сумма */
} HEADER_50;
