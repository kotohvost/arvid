...
#include <linux/malloc.h> /* kmalloc (, GFP_KERNEL) */
...
static avt_unit avttab [MAX_AVT]; /* tape info by unit number */
...
MODULE_AUTHOR("Slavik Levtchenko");
MODULE_DESCRIPTION("ArVid 103X-105X driver by Sl");
MODULE_SUPPORTED_DEVICE(MOD_NAME);

static avt_param *avt_get_param (avt_param *pavtp, avt_unit *punit)
{
  /* Hardware parametrs */
  pavtp->ArvidInsert      = punit->ArvidInsert;
  pavtp->ArvidModel       = punit->ArvidModel;
  pavtp->ArvidModelDetect = punit->ArvidModelDetect;

  pavtp->Phase            = punit->Phase;
  pavtp->SubPhase         = punit->SubPhase;
  pavtp->AutoPhase        = punit->AutoPhase;
  pavtp->BasePort         = punit->BasePort;
  pavtp->ArvidIRQ         = punit->ArvidIRQ;

  pavtp->ValueRK          = punit->ValueRK;
  pavtp->Arv105X          = punit->Arv105X;
  pavtp->DataMode         = punit->DataMode;
  pavtp->HalfDupMode      = punit->HalfDupMode;
  pavtp->HDensity         = punit->HDensity;
  pavtp->FrameMask        = punit->FrameMask;

  pavtp->MemorySize       = punit->MemorySize;
  pavtp->HArvFrame        = punit->HArvFrame;
  pavtp->WArvFrame        = punit->WArvFrame;
  pavtp->NumArvFrame      = punit->NumArvFrame;
  pavtp->SizeArvFrame     = punit->SizeArvFrame;

  /* interrupt */       
  pavtp->countintr        = punit->countintr;
  pavtp->SubFrame         = punit->SubFrame;
  pavtp->SendFrameCount   = punit->SendFrameCount;
  pavtp->ReceiveFrameCount= punit->ReceiveFrameCount;
  
  return pavtp;
}

static void SetDefAvtSettings (avt_unit *punit, int num_dev)
{
  /* Hardware parametrs */
  ...
  punit->AutoPhase = autophase[num_dev]?(autophase[num_dev] & 1) << 2:0; /* SET_AUTO_PHASE */
...
#if 0
static void SendFrameFromInt (avt_unit *punit, word *buf)
{
...
static void ReceiveFrameFromInt (avt_unit *punit, word *buf)
{
...
static void SendFrame (avt_unit *punit, word *buf)
{
  int LastWordsOfBlock = 0x100;
  int LastWords = punit->SizeArvFrame;
  /* dump ((byte*)buf, 0x100); */
...
static void ReceiveFrame (avt_unit *punit, word *buf)
{
...
static void ApplyMode (avt_unit *punit)
{
...
static inline word GetArvidRawFrameCount (avt_unit *punit)
{
  word status = inw (RS1);
...
static inline word GetArvidFrameCount (avt_unit *punit)
{
#if 0
...
static void avt_proc_tasklet (unsigned long data)
{
  word fr_count;
  avt_unit *punit = (avt_unit *)data;
  
  if (!waitqueue_active (&punit->wait))
   {
/*    printk (KERN_CRIT "avtfs:handler:sizeof(wait) %i\n", sizeof(punit->wait) ); */
/*    printk (KERN_CRIT "avtfs:compiler_vershion: %i\n", __GNUC__); */
    return;
   }
...
DECLARE_TASKLET (avt_tasklet, avt_proc_tasklet, 0);

static void Handler (int irq, void *dev_id, struct pt_regs *regs)
{
  avt_unit *punit = dev_id;
  punit->countintr++;
  
  if (!punit->OpenUnit)
    return;

  avt_tasklet.data = (unsigned long)punit;
  tasklet_schedule(&avt_tasklet);
}

static int InitInterrupt (avt_unit *punit)
{
  int rc = 0;
  
  /* unsigned long mask = probe_irq_on (); int probe_irq_off (mask); */
...
static int FreeInterrupt (avt_unit *punit)
{
  int rc = 0;
...
static void SetArvMemory (avt_unit *punit, int subframes, word WordMask)
{
  dword i, j;
...
#if 0
static dword FullTestArvMemory (avt_unit *punit)
{
...
static dword TestArvMemory (avt_unit *punit)
{
  dword i, MemCount = 0;
...
static int AvtProbe (avt_unit *punit)
{
...
static void SetMode (avt_unit *punit, word flags)
{
  int i;
  word mask = 1;
    ...
    if (punit->ArvidModel == 1031)
     {
      ...
      /* Overrided detected value for arvid less tests! */
      punit->NumArvFrame = 8;
  ...
  else 
   { /* irc mode */
    ...
    if (punit->ArvidModel == 1031)
    ...
    else
     {
      if (flags & SET_RECIVE)
        punit->Arv105X = SET_1052;
      else /* into irc send mode set emulation */
...
static void FreeRegion (avt_unit *punit)
{
  if (punit->presource)
   {
...
static int InitUnit (int unit, word flags)
{
...
  printk (KERN_CRIT "avtfs:punit->szbuf %d bytes\n", punit->szbuf);
  ...
  /* printk (KERN_CRIT "avtfs:board %d init, memory on board %d bytes\n", unit, punit->MemorySize); */
  return rc;
}
...
static void FreeUnit (int unit)
{
  ...
  if (punit->ArvidInsert)
   { /* wait clearing arvid cache in send mode */
...
static int avtfs_open (struct inode *inode, struct file *file)
{
...
static int avtfs_release (struct inode *inode, struct file *file)
{
  int unit = NUM(inode->i_rdev);
/*  int type = TYPE(inode->i_rdev); */
...
static int avtfs_ioctl (struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
  int unit = NUM(inode->i_rdev);
/*  int type = TYPE(inode->i_rdev); */
  ...
  switch (cmd)
   {
    ...
    case AVT_GET_PARAM:
     {
      avt_param avtp;     
      avt_get_param (&avtp, punit);
/*      rc = __PUT_USER (avtp, (avt_param *)arg); */ /* compiler 3.0.2 crash here */
      rc = copy_to_user ((avt_param *)arg, &avtp, sizeof (avt_param));
      break;
     } 
...
static ssize_t avtfs_read (struct file *file, char *buffer, size_t count, loff_t *ppos)
{
  avt_unit *punit = file->private_data;
...
/*      printk (KERN_CRIT "avtfs:reading %d bytes\n", rd_count); */
...
/*      printk (KERN_CRIT "punit->ValueRK %04Xh\n", punit->ValueRK); */
	...
	if (rd_count <= count)
	 {
/*	  printk (KERN_CRIT "avtfs:receive frame, set SubFrame %Xh\n", punit->SubFrame); */
/*	  printk (KERN_CRIT "punit->ValueRK %04Xh\n", punit->ValueRK); */
...
static ssize_t avtfs_write (struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
  avt_unit *punit = file->private_data;
...
/*      printk (KERN_CRIT "avtfs:writing %d bytes\n", wr_count); */
/*      interruptible_sleep_on_timeout (&punit->wait, HZ/50); */
      interruptible_sleep_on (&punit->wait);
	...
	if (wr_count <= count)
	 {
	  ...
	  buffer += punit->SizeArvFrame << 1;
/*	  printk (KERN_CRIT "avtfs:send frame, set SubFrame %Xh\n", punit->SubFrame); */
...
static struct file_operations avtfs_fops = {
  read:    avtfs_read,
  write:   avtfs_write,
  open:    avtfs_open,
  release: avtfs_release,
  ioctl:   avtfs_ioctl
};
...
{
  major = register_chrdev (AVT_MAJOR, MOD_NAME, &avtfs_fops);
  
  if (major < 0)
   {
    printk (KERN_WARNING "avtfs:can't get major %d\n", major);
    return major;
   }
